...
bool GDSocket::Send (char* data, int size, int flags)
{
	...
	while(!Done)
	{
		...
		if(timeout > timeGetTime() + Timeout && Bytes == 0)
			return false;
	}
...
void GDSocket::StripEscapeCodes(char* string, char* output)
{
	...
	for(int i=0; i<strlen(string); i++)
	{
		...
		if(string[i] == 109 && SkipEscapeCode == true)
		{
